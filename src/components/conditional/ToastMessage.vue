<script setup lang="ts">
  ...
  const toastMessage = computed<ToastMessage>(() => store.getters['app/toastMessage'])
  
  const onCloseMessage = () => store.dispatch('app/sendToastMessage', null)
</script>
  ...
  <transition name="toast-fade">
    <section v-if="toastMessage" id="toast-message" class="fixed bottom-0 right-0 max-w-md m-8">
      <div class="bg-gray-500 text-blue-500 rounded-lg shadow-md px-8 py-6 pr-12" style="min-width: 240px">
        <ButtonX size="20" class="absolute top-0 right-0 rounded-full p-3" @click.prevent="onCloseMessage" />
